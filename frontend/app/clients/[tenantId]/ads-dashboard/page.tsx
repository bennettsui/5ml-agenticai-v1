  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [kpiRes, aggRes, campRes] = await Promise.all([
        fetch(`${API_BASE}/api/ads/performance/kpis?${queryParams}`),
        fetch(`${API_BASE}/api/ads/performance/aggregated?${queryParams}`),
        fetch(`${API_BASE}/api/ads/performance/campaigns?${queryParams}`),
      ]);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 mt-1">
                  Ad performance dashboard for client: {tenantId}
                </p>
              </div>
            </div>
            <button
