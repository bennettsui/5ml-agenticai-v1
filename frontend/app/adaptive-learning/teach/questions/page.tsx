...
export default function QuestionBankPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await fetch('/api/adaptive-learning/teachers/questions/pending?limit=100');
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/adaptive-learning/teach/questions/pending"
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-16 text-slate-500">
          ...
          <Link href="/adaptive-learning/teach/upload" className="text-purple-400 text-sm underline">Upload a paper →</Link>
        </div>
      )}
    </div>
  );
