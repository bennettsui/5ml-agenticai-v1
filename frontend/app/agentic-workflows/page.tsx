...
// ────────────────────────────────────────────
// Workflow Definitions (with node positions)
// ────────────────────────────────────────────
...
function getEdgePoints(from: NodeDef, to: NodeDef): { x1: number; y1: number; x2: number; y2: number } {
  ...
  // Determine which side to exit/enter based on direction
  if (Math.abs(dx) > Math.abs(dy)) {
    // Horizontal dominant
    if (dx > 0) {
      x1 = from.x + NODE_W; y1 = fc.y;
      x2 = to.x; y2 = tc.y;
    } else {
      x1 = from.x; y1 = fc.y;
      x2 = to.x + NODE_W; y2 = tc.y;
    }
  } else {
    // Vertical dominant
    if (dy > 0) {
      x1 = fc.x; y1 = from.y + NODE_H;
      x2 = tc.x; y2 = to.y;
    } else {
      x1 = fc.x; y1 = from.y;
      x2 = tc.x; y2 = to.y + NODE_H;
    }
  }
  return { x1, y1, x2, y2 };
}
...
export default function AgenticWorkflowsPage() {
  ...
  // Determine connected edges for hover highlight
  const connectedEdges = hoveredNode
    ? active.edges.filter(e => e.from === hoveredNode || e.to === hoveredNode)
    : [];
  ...
  return (
    <div className="min-h-screen bg-[#1a1b2e] text-white">
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* Workflow Info Bar */}
        <div className="flex items-center justify-between mb-4 bg-white/[0.03] border border-white/5 rounded-xl px-5 py-3">
            ...
            >
                  ...
                  return (
                    <g key={idx}>
                      <path
                        d={path}
                        fill="none"
                        stroke={isHighlighted ? '#ffffff' : strokeColors[edge.type] || '#475569'}
                        strokeWidth={isHighlighted ? 2.5 : 1.5}
                        strokeDasharray={edge.type === 'feedback' ? '6 4' : edge.type === 'conditional' ? '4 2' : 'none'}
                        markerEnd={`url(#arrowhead-${isHighlighted ? 'highlight' : edge.type})`}
                      ...
                      {edge.label && (
                        <text
                          ...
                          fill={isDimmed ? '#334155' : edge.type === 'conditional' ? '#f59e0b' : edge.type === 'feedback' ? '#a855f7' : '#64748b'}
              ...
              {active.nodes.map(node => {
                ...
                return (
                  <div
                    key={node.id}
                    className={`absolute select-none transition-all duration-200 ${isDimmed ? 'opacity-20' : ''}`}
                    style={{
                      left: node.x,
                      top: node.y,
                      width: NODE_W,
                  ...
                  >
                    <div
                      className={`w-full h-full rounded-xl border-2 flex items-center gap-2.5 px-3 transition-all duration-200 cursor-default ${
                        isHovered
                          ? 'bg-white/10 border-white/40 shadow-lg shadow-white/5 scale-105'
                          : 'bg-[#1e1f35] border-white/[0.08] hover:border-white/20'
                      }`}
                      style={isHovered ? { borderColor: node.color + '80' } : {}}
                    >
                      <div
                    ...
                    {/* Connection dots */}
                    <div
                      className="absolute -right-1 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full border border-white/20"
                      style={{ backgroundColor: node.color + '60' }}
                    />
                    <div
                      className="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full border border-white/20"
                      style={{ backgroundColor: node.color + '60' }}
                    />
                  </div>
                );
        ...
        {/* Legend */}
        <div className="mt-4 flex items-center gap-6 text-xs text-slate-500 px-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {workflows.map(w => {
              ...
              return (
                <button
                  key={w.id}
                  onClick={() => { setSelected(w.id); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                  className={`text-left bg-white/[0.03] border rounded-xl p-5 transition-all hover:bg-white/[0.06] ${
                    w.id === selected ? 'border-purple-500/30 bg-purple-500/5' : 'border-white/5'
                  }`}
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`p-2 rounded-lg bg-gradient-to-br ${w.gradient}`}>
