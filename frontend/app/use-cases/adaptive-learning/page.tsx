...
const AGENTS = [
  {
    ...
    icon: Brain,
    accent: 'blue',
    description: 'Explains concepts and reflects on session performance with each student.',
    modes: ['EXPLAIN_ONE_QUESTION', 'SESSION_SUMMARY'],
  },
  {
    id: 'teacher',
    label: 'TeacherAgent',
    ...
    icon: Users,
    accent: 'purple',
    description: 'Summarises class mastery data and generates individual student learning profiles.',
    modes: ['CLASS_SUMMARY', 'STUDENT_PROFILE'],
  },
  {
    id: 'question',
    label: 'QuestionAgent',
    ...
    icon: PenLine,
    accent: 'amber',
    description: 'Cleans past-paper questions, estimates difficulty, and tags learning objectives.',
    modes: ['QUESTION_AUTHORING'],
  },
  {
    id: 'studentUx',
    ...
    icon: MessageSquare,
    accent: 'emerald',
    description: 'Generates all in-app copy: welcome screens, session intros, question feedback, and gamification prompts.',
    modes: ['WELCOME', 'SESSION_INTRO', 'QUESTION_FEEDBACK', 'SESSION_SUMMARY_STUDENT', 'GAMIFICATION_EVENT'],
  },
  {
    id: 'teacherGuide',
    ...
    icon: BookMarked,
    accent: 'rose',
    description: 'Writes teacher onboarding guides, step-by-step feature walkthroughs, and FAQ answers.',
    modes: ['INTRO_PAGE', 'STEP_BY_STEP_FEATURE', 'FAQ'],
  },
  {
    id: 'techArch',
    label: 'TechArchAgent',
    ...
    icon: Network,
    accent: 'cyan',
    description: 'Produces Mermaid diagrams — high-level architecture, student session flows, teacher upload sequences.',
    modes: ['HIGH_LEVEL_ARCH', 'SEQUENCE_STUDENT_SESSION', 'SEQUENCE_TEACHER_UPLOAD', 'SEQUENCE_TEACHER_DASHBOARD'],
  },
  {
    id: 'gamification',
    ...
    icon: Gamepad2,
    accent: 'orange',
    description: 'Generates badge messages, mission suggestions, and progress nudges to sustain motivation.',
    modes: ['BADGE_MESSAGE', 'SUGGEST_MISSIONS', 'PROGRESS_NUDGE'],
  },
  {
    id: 'adminReport',
    ...
    icon: BarChart3,
    accent: 'slate',
    description: 'Writes clear, non-technical term reports for principals, panel heads, and school boards.',
    modes: ['TERM_REPORT', 'GRADE_REPORT', 'CLASS_REPORT'],
  },
];
...
const FLOW_STEPS = [
  {
    ...
    body: 'S1–S2 students complete adaptive 20-minute sessions. Questions are chosen to target mastery gaps. Every interaction — correctness, time, hints, self-ratings — is logged.',
    icon: GraduationCap,
    color: 'text-blue-400',
  },
  {
    step: '02',
    title: 'Agents Analyse & Generate',
    body: '8 specialist Claude agents process each event in real time: explaining questions, generating session summaries, updating gamification, and producing teacher insights.',
    icon: Brain,
    color: 'text-indigo-400',
  },
  {
    step: '03',
    title: 'Teachers & Leaders Act',
    body: 'Teachers see live class heatmaps and AI-written student profiles. Principals receive readable term reports — no data expertise needed.',
    icon: Users,
      ...
      <div className="border-b border-slate-800/60 bg-slate-900/40">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/use-cases/adaptive-learning/pitch"
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 space-y-16">
        ...
        <div className="space-y-6">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 bg-indigo-500/15 rounded-xl flex items-center justify-center shrink-0 border border-indigo-500/20">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {STATS.map(s => (
              <div
                key={s.label}
                className="dark:bg-white/[0.03] border border-slate-800/60 rounded-xl p-3 text-center"
              >
        ...
        {/* Platform flow */}
        <div>
          <h2 className="text-base font-semibold text-slate-300 mb-5">How it works</h2>
          <div className="grid md:grid-cols-3 gap-4">
            {FLOW_STEPS.map((step, i) => {
              ...
              return (
                <div
                  key={step.step}
                  className="dark:bg-white/[0.03] border border-slate-800/60 rounded-xl p-5 relative"
                >
                  ...
                  {i < FLOW_STEPS.length - 1 && (
                    <ArrowRight className="absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-700 hidden md:block" />
                  )}
                </div>
              );
            })}
        ...
        {/* 8 Agent cards */}
        <div>
          <h2 className="text-base font-semibold text-slate-300 mb-5">8 Specialist Agents</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {AGENTS.map(agent => {
              ...
              return (
                <div
                  key={agent.id}
                  className="dark:bg-white/[0.03] border border-slate-800/60 rounded-xl p-5 flex flex-col gap-3 hover:dark:bg-white/[0.05] transition-colors"
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {agent.modes.map(mode => (
                      <span
                        key={mode}
                        className="text-[10px] font-mono text-slate-600 bg-slate-800/60 rounded px-1.5 py-0.5"
                      >
                        {mode}
                      </span>
                    ))}
        ...
        {/* Architecture note */}
        <div className="dark:bg-white/[0.03] border border-slate-800/60 rounded-xl p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400 leading-relaxed max-w-3xl">
                ...
                and return strict JSON schemas — no free-form text leaks into the application layer.
                Postgres with pgvector stores mastery state, session history, and question embeddings.
              </p>
