...
function findLifeHouseIndex(lunarMonth, hourBranch) {
...
function calcBaseChart(input) {
  ...
  for (const [star, offset] of Object.entries(majorStarOffsetsFromZiwei)) {
    if (star === "紫微") continue;
    const houseIndex = ((ziweiIndex + offset) % 12 + 12) % 12;
  ...
  for (const [star, offset] of Object.entries(majorStarOffsetsFromTianfu)) {
    ...
    const houseIndex = ((tianfuIndex + offset) % 12 + 12) % 12;
    placeStar(houses, starPositions, star, houseIndex);
  }
  ...
  placeStar(houses, starPositions, "天魁", tianKuiBranch);
  placeStar(houses, starPositions, "天鉞", tianYueBranch);

  // 左輔 and 右弼: offset from life house
  placeStar(houses, starPositions, "左輔", (lifeHouseIndex + 2) % 12);
  placeStar(houses, starPositions, "右弼", (lifeHouseIndex + 10) % 12);
