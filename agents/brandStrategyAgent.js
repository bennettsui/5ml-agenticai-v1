 ...
 */
async function orchestrateBrandStrategy(client_name, brief, options = {}) {
  const {
  ...
  orchestrationLog.push({
    step: 'data_evaluation',
    timestamp: new Date().toISOString(),
    message: 'Evaluating data sufficiency for strategic analysis...'
  });
  ...
  // Step 2: If data is insufficient, orchestrate other agents
  if (!dataEvaluation.sufficient) {
    orchestrationLog.push({
      step: 'orchestration_needed',
      message: `Data gaps identified: ${dataEvaluation.gaps.join(', ')}`,
      action: 'Calling other agents to gather required data...'
    });

    // Call required agents based on gaps
    const gatheredData = {};

    if (dataEvaluation.gaps.includes('brand_research')) {
      orchestrationLog.push({ step: 'calling_research_agent' });
      ...
      try {
        ...
        orchestrationLog.push({ step: 'research_agent_complete', success: true });
      } catch (error) {
        orchestrationLog.push({ step: 'research_agent_error', error: error.message });
      }
    }

    if (dataEvaluation.gaps.includes('customer_insight')) {
      orchestrationLog.push({ step: 'calling_customer_agent' });
      ...
      try {
        ...
        orchestrationLog.push({ step: 'customer_agent_complete', success: true });
      } catch (error) {
        orchestrationLog.push({ step: 'customer_agent_error', error: error.message });
      }
    }

    if (dataEvaluation.gaps.includes('competitor_analysis')) {
      orchestrationLog.push({ step: 'calling_competitor_agent' });
      ...
      try {
        ...
        orchestrationLog.push({ step: 'competitor_agent_complete', success: true });
      } catch (error) {
        orchestrationLog.push({ step: 'competitor_agent_error', error: error.message });
      }
    }
  ...
  // Step 3: Perform strategic analysis with all available data
  orchestrationLog.push({
    step: 'strategic_analysis',
    message: 'Performing comprehensive strategic analysis...'
  });
  ...
  // Step 4: Reflection - Evaluate output quality
  orchestrationLog.push({
    step: 'reflection',
    message: 'Evaluating strategic analysis quality...'
  });

  const qualityCheck = await reflectOnOutput(strategicAnalysis, client_name);
  orchestrationLog.push({
    step: 'reflection_result',
    result: qualityCheck
  });

  // Step 5: If quality is insufficient, retry with more specific guidance
  if (!qualityCheck.acceptable && qualityCheck.retry_count < 2) {
    orchestrationLog.push({
      step: 'retry_with_guidance',
      message: `Quality check failed: ${qualityCheck.reason}. Retrying with specific guidance...`
    });

    const refinedAnalysis = await performStrategicAnalysis(
      client_name,
      brief,
      existingData,
      modelSelection,
  ...
  // Return final strategic analysis with orchestration metadata
  return {
    ...strategicAnalysis,
    _orchestration: {
      mode: 'agentic_ai',
...
/**
 * Evaluate if we have sufficient data for strategic analysis
 */
async function evaluateDataSufficiency(client_name, brief, existingData, conversationHistory) {
  ...
  return {
    ...
    recommendation: gaps.length > 0
      ? `Recommend gathering ${gaps.join(', ')} data before proceeding with strategy.`
      : 'Sufficient data available for strategic analysis.'
  };
}

/**
 * Perform strategic analysis with all available data
 */
async function performStrategicAnalysis(client_name, brief, existingData, modelSelection, modelsUsed, additionalGuidance = '') {
  // Construct context from all available data
  let contextData = `**品牌名稱**: ${client_name}\n**研究簡報**: ${brief}\n\n`;

  if (existingData.brand_research) {
    contextData += `**品牌現狀研究數據**:\n${JSON.stringify(existingData.brand_research, null, 2)}\n\n`;
  }
  ...
  if (additionalGuidance) {
    contextData += `**改進指引**: ${additionalGuidance}\n\n`;
  }

  // Use the existing analyzeBrandStrategy with enhanced context
  return await analyzeBrandStrategy(client_name, contextData, { model: modelSelection });
}

/**
 * Reflection: Evaluate the quality of strategic output
 */
async function reflectOnOutput(strategicAnalysis, client_name) {
  // Goal: Check if we have 5 positioning keywords
  const hasPositioningStatement = strategicAnalysis.strategy_report &&
    strategicAnalysis.strategy_report.includes('Positioning Statement');

  const hasStrategicPriorities = strategicAnalysis.strategy_report &&
    strategicAnalysis.strategy_report.includes('戰略優先級');

  const hasActionBlueprint = strategicAnalysis.strategy_report &&
    strategicAnalysis.strategy_report.includes('行動藍圖');

  // Simple quality check
  const acceptable = hasPositioningStatement && hasStrategicPriorities && hasActionBlueprint;

  if (!acceptable) {
    return {
      acceptable: false,
      reason: 'Missing key strategic components',
      guidance: '請確保包含：1) 定位陳述 2) 戰略優先級 3) 行動藍圖，並提供5個核心定位關鍵字',
      retry_count: 0
    };
  }
  ...
  return {
    acceptable: true,
    reason: 'Strategic analysis meets quality standards',
    retry_count: 0
  };
}
